...
parse_channel_info_from_html() {
    ...
    title=$(grep 'tgme_channel_info_header_title' "$html_file" 2>/dev/null | sed -n 's/.*<span[^>]*>\([^<]*\)<.*/\1/p' | head -1 || true)
    # Description
    ...
    if [ -z "$subs" ]; then
        subs=$(grep 'counter_value' "$html_file" 2>/dev/null | sed -n 's/.*counter_value[^>]*>\([^<]*\)<.*/\1/p' | head -1 || true)
    fi
