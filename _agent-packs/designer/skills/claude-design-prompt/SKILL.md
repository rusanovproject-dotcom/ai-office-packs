...
**⚠️ ЭТО ОПЦИЯ, НЕ DEFAULT.** По умолчанию Дизайнер **сам делает** картинку через claude.ai/design и отдаёт клиенту готовое. Этот скилл запускается только когда:
- клиент прямо попросил «дай промт для claude.ai/design, я сам вставлю»
...
Твоя задача (в режиме опции) — превратить задачу клиента в мега-промт для claude.ai/design. 2-3 абзаца. Клиент копирует → вставляет → получает результат.
